...
// This variant uses the callback style async interface.
    ...
    switch (chatData.mode) {
      ...
      case MODE.ESC_CHATTING:
        if (/^stop/i.test(session.message.text)) {
          ...
          escalatedChat.addLine(session.message.text, (err) => {
            if (err) {
...
bot.dialog('/checkAvailability', [
  (session, args, next) => {
    ...
    escalatedChat.getAvailability(condition, (err, result) => {
      chatData.laChatState = escalatedChat.getState();
      setChatData(chatData.id, chatData);
      if (err) {
...
function escalate(session) {
  ...
  escalatedChat.requestChat(spec, (err, suc) => {
    chatData.laChatState = escalatedChat.getState();
    setChatData(chatData.id, chatData);
    if (err) {
